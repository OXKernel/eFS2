...
inode_rtvl_t inode_dev_open(char *path, int *dev)
{
  ...
  if(master->magic != INODE_MAGIC) {
    // Magic number wasn't there, try the first sector.
    if(block_read(*dev,0,(char *)master) != BLOCK_OK) {
      errno = EACCES;
      dev_close(*dev);
      printk("inode_dev_open:: failed to read master BLOCK_START=0\n");
      return INODE_INIT;
    }
...
inode_rtvl_t inode_set_timestamps(inode_t *inode)
{
  ...
  inode->create_time = ktime();
  inode->modified_time = ktime();
  inode->accessed_time = ktime();
